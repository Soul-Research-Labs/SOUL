...
import "../../contracts/security/PILTimelock.sol";

/**
 * @title PILTimelock Test Suite
 * @notice Comprehensive tests for time-locked administrative operations
 */
contract PILTimelockTest is Test {
    PILTimelock public timelock;
    ...
    function test_getOperationStatus() public {
        ...
        PILTimelock.OperationStatus status = timelock.getOperationStatus(
            operationId
        );
        assertEq(
            uint256(status),
            uint256(PILTimelock.OperationStatus.Pending),
