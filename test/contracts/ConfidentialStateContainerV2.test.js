...
describe("ConfidentialStateContainerV2", function () {
  ...
  beforeEach(async function () {
    ...
    const Verifier = await ethers.getContractFactory("contracts/core/Groth16VerifierBLS12381.sol:Groth16VerifierBLS12381");
    verifier = await Verifier.deploy();
    await verifier.waitForDeployment();

    // Deploy container (use V2 fully qualified path)
    const Container = await ethers.getContractFactory("contracts/core/ConfidentialStateContainerV2.sol:ConfidentialStateContainer");
  ...
  describe("State Registration", function () {
    ...
    beforeEach(function () {
      commitment = ethers.keccak256(ethers.toUtf8Bytes("commitment1"));
    ...
    it("Should revert on empty encrypted state", async function () {
      const emptyBytes = "0x";
      await expect(
        container.connect(user1).registerState(emptyBytes, commitment, nullifier, proof, publicInputs)
  ...
  describe("Admin Functions", function () {
    it("Should update verifier", async function () {
      const NewVerifier = await ethers.getContractFactory("contracts/core/Groth16VerifierBLS12381.sol:Groth16VerifierBLS12381");
