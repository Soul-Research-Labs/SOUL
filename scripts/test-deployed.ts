/**
 * PIL v3 Deployed Contract Integration Tests
 * 
...
async function main() {
  console.log("\n" + "=".repeat(80));
  console.log("PIL v3 DEPLOYED CONTRACT INTEGRATION TESTS");
  ...
  // Test 8: PILAtomicSwapV2
  await test("PILAtomicSwapV2 - Contract accessible", async () => {
    const code = await publicClient.getCode({ address: ADDRESSES.atomicSwap });
    if (!code || code === "0x") throw new Error("No code at address");
  });

  // Test 9: PILComplianceV2
  await test("PILComplianceV2 - Contract accessible", async () => {
    const code = await publicClient.getCode({ address: ADDRESSES.compliance });
    if (!code || code === "0x") throw new Error("No code at address");
  });

  console.log("\n📦 Testing PIL v2 Primitives...\n");
